...
def train_epoch(opt: FractionalAdam,
    ...
    for i in range(epoch_size):
        ...
        log_adam_behavior(gaussians=gaussians,adam_optimizer=opt,step=i)
...
def log_adam_behavior(gaussians, adam_optimizer, step, log_file="adam_log.json"):
    import json

    log_data = {
        "step": step,
        "gaussians": {
            "parameters": gaussians.detach().cpu().tolist() if gaussians is not None else None
        },
    ...
    for param_group in adam_optimizer.param_groups:
        for param in param_group['params']:
            if param in adam_optimizer.state:
                ...
                param_data = {
                    "gradients": param.grad.cpu().tolist() if param.grad is not None else None,
                    "m_t": state['exp_avg'].cpu().tolist(),
                    "v_t": state['exp_avg_sq'].cpu().tolist(),
                    "adam_update": (-param_group['lr'] * state['exp_avg'] / (torch.sqrt(state['exp_avg_sq']) + 1e-8)).cpu().tolist(),
                    "before_update": param.data.cpu().tolist(),
                    "after_update": (param.data - param_group['lr'] * state['exp_avg'] / (torch.sqrt(state['exp_avg_sq']) + 1e-8)).cpu().tolist(),
                }
                log_data["optimizer_parameters"].append(param_data)
