...
std::pair<torch::Tensor, torch::Tensor> radix_sort_pairs(
  ...
  } else if (keys.scalar_type() == torch::kFloat32 ) {
    sort_helper<float, int32_t>(
      keys.data_ptr<float>(), values.data_ptr<int32_t>(), 
      keys_out.data_ptr<float>(), values_out.data_ptr<int32_t>(),
      keys.size(0), begin_bit, end_bit);
