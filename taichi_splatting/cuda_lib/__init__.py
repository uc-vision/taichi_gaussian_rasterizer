...
def radix_argsort(keys:torch.Tensor, start_bit=0, end_bit=None):
  
  keys, idx = radix_sort_pairs(keys, 
    torch.arange(keys.shape[0], device=keys.device, dtype=torch.int32), start_bit, end_bit)
